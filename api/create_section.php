...
if (isset($_POST['pageid']) || isset($_POST['title']) || isset($_POST['content']) || isset($_POST['sectiontemplates']) || isset($_POST['file'])) {
    ...
    $size = $_FILES['file']['size'];
     ...
     if ($size > 10000000) { // file shouldn't be larger than 10Megabyte
        ...
        $post->pageid =$_POST['pageid'];
        $post->title=$_POST['title'];
        $post->content=$_POST['content'];
        $post->sectiontemplate=$_POST['sectiontemplates'];
        ...
        $post->sectiontemplate  = $data->sectiontemplates; */

        

        //create post
        if ($post->section()) {
            $returnData = msg(1, 201, "Section created.");
        } else {
            $returnData = msg(1, 422, "Section not created.");
        }
